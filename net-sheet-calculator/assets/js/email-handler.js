...
class EmailHandler {
	...
	_initializeElements($container) {
		...
		const $emailAction = $container.find('.nsc-action').has('.nsc-button--send');
		this.$error = $emailAction.find('.nsc-action__message');
	...
	_bindEvents() {
		this.$input
			.on('input', (event) => {
				...
				if (this.$input.hasClass('nsc-input--error') && this.isValid) {
					this.hideError();
				}
			})
			.on('blur', () => {
