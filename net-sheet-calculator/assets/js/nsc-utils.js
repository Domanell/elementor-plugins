/**
 * Utility functions for Net Sheet Calculator
 * Common helper functions used across the calculator
 */
const NSCUtils = (function ($) {
	'use strict';
	...
	return {
		...
		updateCalculatedField: ($calculator, field, value = 0) => {
			$calculator.find(`.nsc-input[data-field="${field}"]`).val(NSCUtils.formatCurrency(value));
		},

		updateTextOutput: ($calculator, dataSelector, value) => {
			$calculator.find(`[data-field="${dataSelector}"]`).text(NSCUtils.formatCurrency(value));
		},
